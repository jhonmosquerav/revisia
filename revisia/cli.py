...
def _cmd_new(args: argparse.Namespace) -> int:
    ...
    print(f"    3. Valida:  revisia validate {dest}")
    print(f"    4. Ejecuta: revisia run {dest} --brain cerebro")
    print(f"    5. Audita:  revisia audit runs/{args.slug}-<fecha>")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--version", action="version", version=f"revisia {__version__}")
